...
- 入口 `/admin/page-management` 是頁面列表：只列子頁面（不顯示首頁列、無首頁設定，因 `/` 固定維護頁）。每列動作：預覽、編輯頁面、編輯後綴（彈窗改網址 slug）、停用／啟用（取代刪除，停用後前台該 slug 回 404、列表顯示「已停用」標記，內容保留可再啟用）。不使用原生 confirm。
