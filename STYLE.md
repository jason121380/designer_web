...
- 入口 `/admin/page-management` 是頁面列表：只列子頁面（不顯示首頁列、無首頁設定，因 `/` 固定維護頁）。每列動作：預覽（→ /{slug}/web）、一頁式（編輯 OnePage）、連結頁（編輯 linktree）、後綴（彈窗改網址 slug）、停用／啟用（取代刪除，停用後前台該 slug 各頁回 404、列表顯示「已停用」標記，內容保留可再啟用）。不使用原生 confirm。
- 連結頁（`/{slug}/links`）：linktree 風格，置中單欄、頭像圓形、名稱＋簡介、垂直全寬圓角連結按鈕、底部社群 icon；名稱／主色（`var(--brand)`）／社群沿用一頁式的品牌與聯絡資訊。
