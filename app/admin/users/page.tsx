...
export default function UsersPage() {
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="bg-white border border-gray-100 p-6 mb-8 rounded-sm">
            ...
            {[
              ...
              <div key={key}>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 focus:border-rose-brand outline-none px-3 py-2 text-sm"
                />
              </div>
            ))}
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-200 focus:border-rose-brand outline-none px-3 py-2 text-sm bg-white"
              >
                ...
                <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-black rounded-full flex items-center justify-center text-white text-sm font-serif flex-shrink-0">
                        {user.name[0]}
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4 hidden md:table-cell">
                    <span className={`inline-flex px-2 py-0.5 text-xs rounded-sm font-medium ${ROLE_COLORS[user.role]}`}>
                      {ROLE_LABELS[user.role]}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-gray-500 hidden lg:table-cell">{user._count.articles}</td>
                  <td className="px-6 py-4 hidden lg:table-cell">
                    <span className={`inline-flex px-2 py-0.5 text-xs rounded-sm ${user.active ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
                      {user.active ? "啟用" : "停用"}
                    </span>
                  </td>
