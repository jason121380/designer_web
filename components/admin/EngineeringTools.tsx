...
export default function EngineeringTools() {
  ...
  const [result, setResult] = useState<{ scanned: number; added: number } | null>(null);
  ...
  async function backfill() {
    ...
    try {
      ...
      setResult({ scanned: body.scanned ?? 0, added: body.added ?? 0 });
      toast.success(`回填完成：新增 ${body.added ?? 0} 筆`);
    } catch (error) {
      ...
      <div className="border border-gray-200 bg-white rounded-lg p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-gray-500">掃描所有頁面內容，把尚未列在媒體庫的媒體網址補進來（主要用於這次改版前上傳、沒有紀錄的舊影片）。安全可重複執行，已存在的不會重複新增。</p>
            ...
            {result && (
              <p className="mt-3 text-sm text-gray-600">掃描 {result.scanned} 個網址，新增 {result.added} 筆到媒體庫。</p>
            )}
          </div>
        </div>
      </div>
