...
export default function RichTextEditor({ content, onChange, placeholder = "開始撰寫內容..." }: Props) {
  ...
  const setLink = useCallback(() => {
    const url = window.prompt("請輸入連結網址（站內錨點請用 #錨點ID，例：#section-1）：");
    if (url === null) return;
    if (url === "") { editor?.chain().focus().extendMarkRange("link").unsetLink().run(); return; }
    editor?.chain().focus().extendMarkRange("link").setLink({ href: url }).run();
  }, [editor]);

  const setHeadingAnchor = useCallback(() => {
    if (!editor) return;
    if (!editor.isActive("heading")) {
      alert("請先把游標放在要設錨點的「標題」上");
      return;
    }
    ...
    editor
      .chain()
      .focus()
      .updateAttributes("heading", { id: id.trim() || null })
      .run();
  }, [editor]);
  ...
  return (
    <div className="border border-gray-200 rounded-sm overflow-hidden">
      ...
      <div className="border-b border-gray-200 bg-gray-50 p-2 flex flex-wrap gap-1.5">
        ...
        <ToolbarButton onClick={setLink} active={editor.isActive("link")} title="插入連結（站內錨點用 #ID）">🔗</ToolbarButton>
        <ToolbarButton onClick={setHeadingAnchor} active={editor.isActive("heading") && !!editor.state.selection.$from.parent.attrs?.id} title="設定標題錨點（手動目錄用）">⚓</ToolbarButton>
