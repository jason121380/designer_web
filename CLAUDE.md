...
1. 後台側欄只保留「頁面管理」與「用戶管理」兩個入口（用戶管理僅 ADMIN 可見，功能限「列出帳號 + 新增帳號 + 編輯帳號（登入帳號/名稱/密碼）」，新增時可選 EDITOR/ADMIN 角色）。不要重新加入總覽、文章、分類、標籤、媒體庫、流量分析或工程工具，也不要把用戶管理擴充成刪除帳號或獨立的角色管理模組。
2. 頁面管理區塊順序必須跟前台一致。
